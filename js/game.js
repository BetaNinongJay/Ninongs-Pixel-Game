...
document.addEventListener('keydown', e => {
    ...
    if (k === ' ' && !isAttacking && !isHit && !isDead && !soldierDead && !debugMapMode && !isPaused && !showControlGuide && !showStatsPanel) {
        startAttack();
    }
    if (k === 'm' && !isPaused && !showControlGuide && !showStatsPanel) {
...
canvas.addEventListener('mousedown', e => {
    ...
    // 1) HUD Buttons (Pause, Help Question Mark, & Status Orb Button)
    const pauseBtnX = LOGICAL_WIDTH - 52, pauseBtnY = 8, btnSize = 44;
    const helpBtnX = LOGICAL_WIDTH - 104, helpBtnY = 8;
    const statsBtnX = LOGICAL_WIDTH - 156, statsBtnY = 8;

    // Check Status Button click (beside question mark button)
    if (pos.x >= statsBtnX && pos.x <= statsBtnX + btnSize && pos.y >= statsBtnY && pos.y <= statsBtnY + btnSize) {
        showStatsPanel = !showStatsPanel;
        toggleControlGuideUI(false);
        isPaused = false;
        return;
    ...
    // 2) If Stats Panel Modal is open
    if (showStatsPanel) {
        const statsW = 430, statsH = 480;
        const statsX = (LOGICAL_WIDTH - statsW) / 2;
        const statsY = (LOGICAL_HEIGHT - statsH) / 2;
        const closeX = statsX + statsW - 62, closeY = statsY + 44, closeSize = 28;

        if ((pos.x >= closeX && pos.x <= closeX + closeSize && pos.y >= closeY && pos.y <= closeY + closeSize) ||
            pos.x < statsX || pos.x > statsX + statsW || pos.y < statsY || pos.y > statsY + statsH) {
            showStatsPanel = false;
        }
        return;
    }

    // 3) If Control Guide Modal is open
    if (showControlGuide) {
    ...
    // 4) If Pause Menu is active (PLAY / LOAD / EXIT)
    if (isPaused) {
...
function loadGameProgress() {
    if (savedState) {
        ...
        xpToNextLevel = savedState.xpToNextLevel || BASE_XP_TO_LEVEL;
        currentMaxHP = savedState.currentMaxHP || SOLDIER_MAX_HP;
        currentDamage = savedState.currentDamage || SOLDIER_DAMAGE;
        totalKills = savedState.totalKills || 0;
        goldCount = savedState.goldCount || 0;
        animatedSoldierHP = soldierHP;
        animatedXPRatio = (playerLevel >= MAX_LEVEL) ? 1 : (playerXP / xpToNextLevel);
...
function update() {
    ...
    // If game is paused, guide open, or stats panel open, freeze game state updates
    if (isPaused || showControlGuide || showStatsPanel) return;
...
function render() {
    ...
    // ── Standalone Gold Counter & Stats Shortcut Badge ─────────────────────────
    const goldX = 16;
    ...
    // Sleek Level badge in top-left
    const lvBadgeX = goldX + 115;
    ctx.fillStyle = 'rgba(14, 18, 30, 0.82)';
    ctx.strokeStyle = '#8cf';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.roundRect(lvBadgeX, goldY - 13, 72, 18, 4);
    ctx.fill();
    ctx.stroke();

    ctx.font = 'bold 10px "Cinzel", "Inter", sans-serif';
    ctx.textAlign = 'center';
    ctx.fillStyle = '#8cf';
    const lvText = playerLevel >= MAX_LEVEL ? `LV.MAX` : `LV.${playerLevel}`;
    ctx.fillText(lvText, lvBadgeX + 36, goldY - 1);

    ctx.restore();

    // ── HUD Buttons: Status Icon, Pause Icon & Help Question Mark Icon ─────────
    const pauseBtnX = LOGICAL_WIDTH - 52, pauseBtnY = 8, btnSize = 44;
    const helpBtnX = LOGICAL_WIDTH - 104, helpBtnY = 8;
    const statsBtnX = LOGICAL_WIDTH - 156, statsBtnY = 8;
    ...
    // 1) Render Status Button (Glowing Diamond Orb from freefantasy.png)
    ctx.fillStyle = 'rgba(15, 12, 25, 0.85)';
    ctx.strokeStyle = showStatsPanel ? '#e8c97a' : 'rgba(255, 255, 255, 0.3)';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.roundRect(statsBtnX, statsBtnY, btnSize, btnSize, 8);
    ctx.fill();
    ctx.stroke();

    ctx.drawImage(healthBarUI,
        BTN_STATS_SX, BTN_STATS_SY, BTN_SRC_SIZE, BTN_SRC_SIZE,
        statsBtnX + 8, statsBtnY + 8, btnSize - 16, btnSize - 16
    );

    // 2) Render Question Mark (?) Button
    ctx.fillStyle = 'rgba(15, 12, 25, 0.85)';
    ...
    // ── STATS PANEL MODAL (Opened by Status Orb Button or 'C' Key) ────────
    if (showStatsPanel) {
        ctx.save();
        ctx.fillStyle = 'rgba(5, 4, 10, 0.72)';
        ctx.fillRect(0, 0, canvas.width, canvas.height);

        const statsW = 390, statsH = 420;
        const statsX = (LOGICAL_WIDTH - statsW) / 2;
        const statsY = (LOGICAL_HEIGHT - statsH) / 2;

        // Clean dark panel with golden border (no wooden board background)
        ctx.imageSmoothingEnabled = false;
        const innerX = statsX, innerY = statsY, innerW = statsW, innerH = statsH;
        ctx.fillStyle = 'rgba(14, 10, 20, 0.94)';
        ctx.strokeStyle = '#e8c97a';
        ctx.lineWidth = 2.5;
        ctx.beginPath();
        ...
        ctx.textBaseline = 'middle';
        ctx.fillStyle = '#e8c97a';
        ctx.fillText('⚔ HERO STATUS & STATS ⚔', LOGICAL_WIDTH / 2, innerY + 24);

        // Header Separator Line
        ctx.strokeStyle = 'rgba(232, 201, 122, 0.3)';
        ctx.beginPath();
        ctx.moveTo(innerX + 16, innerY + 42);
        ctx.lineTo(innerX + innerW - 16, innerY + 42);
        ctx.stroke();

        // Close Button [X]
        const closeX = innerX + innerW - 28, closeY = innerY + 12, closeSize = 22;
        ...
        // Rank Titles array based on level
        const rankTitles = ['Novice Recruit', 'Slayer Apprentice', 'Veteran Fighter', 'Dungeon Knight', 'Champion Warrior', 'Hero of Realm', 'Dungeon Master', 'High Warlord', 'Grand Champion', 'Legendary Conqueror'];
        const currentRank = rankTitles[Math.min(playerLevel - 1, rankTitles.length - 1)];

        let curY = innerY + 62;

        // 1) PLAYER LEVEL & RANK BADGE
        ctx.fillStyle = 'rgba(232, 201, 122, 0.1)';
        ctx.strokeStyle = 'rgba(232, 201, 122, 0.4)';
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.roundRect(innerX + 16, curY, innerW - 32, 38, 6);
        ctx.fill();
        ctx.stroke();
        ...
        ctx.textAlign = 'left';
        ctx.fillStyle = '#8cf';
        ctx.fillText(`LEVEL ${playerLevel}`, innerX + 28, curY + 19);

        ctx.font = 'bold 12px "Cinzel", "Inter", sans-serif';
        ctx.textAlign = 'right';
        ctx.fillStyle = '#f5e4b8';
        ctx.fillText(currentRank, innerX + innerW - 28, curY + 19);

        curY += 50;

        // 2) EXP BAR STATUS (Moved into Stats Panel as requested!)
        ctx.font = 'bold 11px "Cinzel", sans-serif';
        ctx.textAlign = 'left';
        ctx.fillStyle = '#e8c97a';
        ctx.fillText('EXP STATUS:', innerX + 18, curY);

        ctx.textAlign = 'right';
        ctx.fillStyle = '#8cf';
        const expPercentStr = playerLevel >= MAX_LEVEL ? 'MAX LEVEL' : `${playerXP} / ${xpToNextLevel} XP (${((playerXP / xpToNextLevel) * 100).toFixed(1)}%)`;
        ctx.fillText(expPercentStr, innerX + innerW - 18, curY);

        curY += 10;

        // Render XP Bar sprite in Stats Panel
        const xpBarW = innerW - 36;
        const xpBarH = 22;
        if (xpBarSheet && xpBarSheet.complete) {
            ctx.drawImage(xpBarSheet,
                XP_BAR_FRAME_SX, XP_BAR_FRAME_SY, XP_BAR_FRAME_SW, XP_BAR_FRAME_SH,
                innerX + 18, curY, xpBarW, xpBarH
            );
            ...
            if (xpRatio > 0.01) {
                ...
                const scaleXFactor = xpBarW / XP_BAR_FRAME_SW;
                const scaleYFactor = xpBarH / XP_BAR_FRAME_SH;
                ctx.drawImage(xpBarSheet,
                    fillSrcX, XP_BAR_FILL_SY, XP_BAR_FILL_SW, XP_BAR_FILL_SH,
                    innerX + 18 + 3 * scaleXFactor, curY + 3 * scaleYFactor,
                    XP_BAR_FILL_SW * scaleXFactor, XP_BAR_FILL_SH * scaleYFactor
                );
            }
        }
        ...
        // 3) RECENT HEALTH (HP) STATUS
        ctx.font = 'bold 11px "Cinzel", sans-serif';
        ctx.textAlign = 'left';
        ctx.fillStyle = '#e8c97a';
        ctx.fillText('HEALTH STATUS:', innerX + 18, curY);

        ctx.textAlign = 'right';
        ctx.fillStyle = '#ff6b6b';
        const hpPercentStr = `${Math.round(animatedSoldierHP)} / ${currentMaxHP} HP (${Math.round((soldierHP / currentMaxHP) * 100)}%)`;
        ctx.fillText(hpPercentStr, innerX + innerW - 18, curY);

        curY += 10;

        // Render Pixel UI Health Bar sprite in Stats Panel
        const hpBarW = innerW - 36;
        const hpBarH = 16;
        if (pixelUISheet && pixelUISheet.complete) {
            const hpRatio = Math.max(0, Math.min(1, animatedSoldierHP / currentMaxHP));
            let stageIdx = 4;
            ...
            ctx.drawImage(pixelUISheet,
                PIXEL_HB_STAGES[stageIdx], PIXEL_HB_Y, PIXEL_HB_SW, PIXEL_HB_SH,
                innerX + 18, curY, hpBarW, hpBarH
            );
        }

        curY += 32;

        // 4) ATTACK POWER & COMBAT STATS GRID
        const gridW = (innerW - 44) / 2;
        const gridH = 50;
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.roundRect(innerX + 18, curY, gridW, gridH, 6);
        ctx.fill();
        ctx.stroke();
        ...
        ctx.fillText('⚔ ATTACK POWER', innerX + 18 + gridW / 2, curY + 18);
        ctx.font = 'bold 15px "Inter", sans-serif';
        ctx.fillStyle = '#fff';
        ctx.fillText(`${currentDamage} DMG`, innerX + 18 + gridW / 2, curY + 37);
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.roundRect(innerX + 26 + gridW, curY, gridW, gridH, 6);
        ctx.fill();
        ctx.stroke();
        ...
        ctx.fillText('❤️ MAX HEALTH', innerX + 26 + gridW + gridW / 2, curY + 18);
        ctx.font = 'bold 15px "Inter", sans-serif';
        ctx.fillStyle = '#fff';
        ctx.fillText(`${currentMaxHP} HP`, innerX + 26 + gridW + gridW / 2, curY + 37);

        curY += 60;

        // 5) GOLD & DUNGEON SLAIN KILLS GRID
        // Gold Box
        ...
        ctx.textAlign = 'center';
        ctx.fillStyle = '#ffd700';
        ctx.fillText('💰 GOLD COINS', innerX + 18 + gridW / 2, curY + 18);
        ctx.font = 'bold 15px "Inter", sans-serif';
        ctx.fillStyle = '#fff';
        ctx.fillText(`${goldCount} Gold`, innerX + 18 + gridW / 2, curY + 37);
        ...
        ctx.fillText('☠ SKELETON KILLS', innerX + 26 + gridW + gridW / 2, curY + 18);
        ctx.font = 'bold 15px "Inter", sans-serif';
        ctx.fillStyle = '#fff';
        ctx.fillText(`${totalKills} Slain`, innerX + 26 + gridW + gridW / 2, curY + 37);

        // Level Up active glow notice inside panel
        if (levelUpEffect > 0) {
            ctx.font = 'bold 12px "Cinzel", sans-serif';
            ctx.textAlign = 'center';
            ctx.fillStyle = '#ffd700';
            ctx.fillText('✨ LEVEL UP BONUS ACTIVE! (+15 HP, +2 DMG) ✨', LOGICAL_WIDTH / 2, innerY + innerH - 14);
        }
    ...
    if (showControlGuide) {
        ...
        const controls = [
            ...
            { key: 'C', desc: 'Toggle Player Stats & EXP status panel' },
